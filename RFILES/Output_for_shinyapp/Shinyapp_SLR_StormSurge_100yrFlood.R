#########################################################################
# file: Shinyapp_SLR_StormSurge_100yrFlood.R
#------------------------------------------------------------------------
...
#
# NOTE: This script takes ~1 hours to run
#------------------------------------------------------------------------
...
# NOTE: THIS differs from the paper by NOT Accounting for dams and reservoirs
...
write.csv(projSLRDamsRes,"Sea_level_proj_anomalies.csv")
write.csv(freq_mat,"Storm_surge_freq.csv")
...
write.csv(new.probs2020,"NewUncertaintyProbs2020.csv",na="0")
uncertainty.probs.table2020 <- read.csv("NewUncertaintyProbs2020.csv")
...
write.csv(accountUNC_2020,"StormFreqAccountUNC2020.csv")
...
small.num2050 = fit_q_year[2]/100 + min(projSLRDamsRes[,2])
end.num2050 = round(meanyear2050/1.1, 1)
num.range12050 = seq(small.num2050, end.num2050, length.out=20)
# To 1 and 1/2 the 100-yr storm surge plus mean sea-level anomaly
max.pot.num2050 = meanyear2050 + mean(projSLRDamsRes[,2])
...
# Set all NAs to 0 since they are smaller than 1:100,000. This gives us a conservative estimate.
write.csv(new.probs2050,"NewUncertaintyProbs2050.csv",na="0")
uncertainty.probs.table2050 <- read.csv("NewUncertaintyProbs2050.csv")
...
#Find the new 100-yr probability
new.returnLevel2050 <- which(average.uncertainty.probs2050 >= 0.01)
...
accountUNC_2050[1,3] <- max.returnL2050 # + DamsReservoirs #The year 2080 is the 221st number

colnames(accountUNC_2050, do.NULL = FALSE)
colnames(accountUNC_2050) = c("ReturnPeriod", "ReturnLevel", "100LevelNum")

write.csv(accountUNC_2050,"StormFreqAccountUNC2050.csv")
...
small.num2080 = fit_q_year[2]/100 + min(projSLRDamsRes[,3])
end.num2080 =  round(meanyear2080, 1)
num.range12080 = seq(small.num2080, end.num2080, length.out=20)
# To 1 and 1/2 the 100-yr storm surge plus mean sea-level anomaly
max.pot.num2080 =  meanyear2080 + mean(projSLRDamsRes[,3])
...
# Set all NAs to 0 since they are smaller than 1:100,000. This gives us a conservative estimate.
write.csv(new.probs2080,"NewUncertaintyProbs2080.csv",na="0")
uncertainty.probs.table2080 <- read.csv("NewUncertaintyProbs2080.csv")
...
write.csv(accountUNC_2080,"StormFreqAccountUNC2080.csv")
...
# 
# # Global mean sea-level
# load("Workspace/SanFranSLR_StormSurge_Analysis2.RData")
# 
...
write.csv(accountUNC_2100,"StormFreqAccountUNC2100.csv")
##################### END #####################
